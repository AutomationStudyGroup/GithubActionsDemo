...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'TheEnd.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'TheEnd.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'TheEnd.Client.ProtectedToken'
