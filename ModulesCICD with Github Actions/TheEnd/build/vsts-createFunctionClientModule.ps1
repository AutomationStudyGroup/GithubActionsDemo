...
$parentModule = 'TheEnd'
if (-not $ModuleName) { $ModuleName = 'TheEnd.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\TheEnd'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\TheEnd.psd1"
$prereqHash = @{
